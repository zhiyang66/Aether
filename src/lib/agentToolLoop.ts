...
/** Shared pane targeting: serials restart at #1 per tab — always prefer T{tab}:#{n}. */
const PANE_REF_PROP = {
  type: "string",
  description:
    '跨标签窗格引用，格式 "T1:#1"（推荐）。标签序号从左到右从 1 起；# 序号每标签独立从 1 起。跨标签操作必须用此字段，勿只用 serial。',
} as const;

const PANE_SERIAL_PROP = {
  type: "integer",
  description:
    "仅当前焦点标签内的窗格 #N。跨标签请用 pane=\"T1:#1\"。省略 serial 与 pane 时使用焦点窗格。",
} as const;
...
export const AGENT_TOOLS_OPENAI = [
  {
    ...
    function: {
      name: "list_panes",
      description:
        "列出所有终端窗格，格式 T{标签}:#{序号}、标题、shell、cwd、是否焦点。跨标签操作前必先调用；引用时用完整 T1:#1，不要只用 #1。",
  ...
  },
  {
    ...
    function: {
      name: "read_pane",
      description:
        "读取指定窗格最近的终端输出（已脱敏）。blocks=true 时返回结构化命令块列表（命令/退出码/耗时），并附最近一块的输出。跨标签请传 pane=\"T1:#1\"。",
      parameters: {
        type: "object",
        properties: {
          pane: PANE_REF_PROP,
  ...
  },
  {
    ...
    function: {
      name: "close_pane",
      description: "关闭指定窗格（至少保留一个）。跨标签用 pane=\"T1:#1\"。",
      parameters: {
        type: "object",
        properties: {
          pane: PANE_REF_PROP,
  ...
  },
  {
    ...
    function: {
      name: "focus_pane",
      description:
        "聚焦到指定窗格（会切换标签）。跨标签必须 pane=\"T1:#1\"；仅 serial 只在当前标签内查找。",
      parameters: {
        type: "object",
        properties: {
          pane: PANE_REF_PROP,
  ...
  },
  {
    ...
    function: {
      name: "clear_pane",
      description: "清屏指定或焦点窗格的终端显示。跨标签用 pane=\"T1:#1\"。",
      parameters: {
        type: "object",
        properties: {
          pane: PANE_REF_PROP,
...
/**
 * Resolve a pane from tool args.
 * Prefer `pane` / `pane_ref` / `target` as "T1:#1"; fall back to `serial` (active-tab first).
 * Omitting both → focus pane.
 */
function resolveTargetPane(
  st: ReturnType<typeof useWorkbenchStore.getState>,
  args: Record<string, unknown>,
): { leaf: LeafPane | null; label: string; explicit: boolean } {
  const paneRaw =
    args.pane != null
      ? String(args.pane).trim()
      : args.pane_ref != null
        ? String(args.pane_ref).trim()
        : args.target != null
  ...
  if (paneRaw) {
    // Direct T{n}:#{m}
    const leaf = st.resolvePaneRef?.(paneRaw) ?? null;
    if (leaf) return { leaf, label: paneRaw, explicit: true };
    // Also accept bare "T1:1"
    const m = paneRaw.match(/^t(\d+)\s*[:：]\s*#?(\d+)$/i);
    if (m) {
      const tab = st.tabs[Number(m[1]) - 1];
      const serial = Number(m[2]);
      if (tab && Number.isFinite(serial)) {
        const hit = findLeafBySerial(tab.layout, serial);
        if (hit) return { leaf: hit, label: `T${m[1]}:#${serial}`, explicit: true };
      }
      return { leaf: null, label: paneRaw, explicit: true };
    }
    // pane="1" / pane="#1"
    const m2 = paneRaw.match(/^#?(\d+)$/);
    if (m2) {
      const serial = Number(m2[1]);
      const hit = st.resolveSerial(serial);
      return { leaf: hit, label: `#${serial}`, explicit: true };
    }
    return { leaf: null, label: paneRaw, explicit: true };
  }

  if (args.serial != null && args.serial !== "") {
    const serial = Number(args.serial);
    if (Number.isFinite(serial)) {
      return {
        leaf: st.resolveSerial(serial),
        label: `#${serial}`,
        explicit: true,
      };
    }
  ...
  const focus = st.activePane();
  return {
    leaf: focus,
    label: focus ? `#${focus.serial}` : "焦点",
    explicit: false,
  };
}

/** Switch tab + focus so writes land on the intended pane (cross-tab safe). */
...
/** Human label T{i}:#{serial} for a leaf (best-effort). */
function paneLabelOf(
  st: ReturnType<typeof useWorkbenchStore.getState>,
  leaf: LeafPane,
): string {
  for (let i = 0; i < st.tabs.length; i++) {
    if (collectLeaves(st.tabs[i].layout).some((l) => l.id === leaf.id)) {
      return `T${i + 1}:#${leaf.serial}`;
    }
  }
  return `#${leaf.serial}`;
}
...
export async function executeAgentTool(
  ...
  if (name === "list_panes") {
    ...
    st.tabs.forEach((tab, ti) => {
      for (const leaf of collectLeaves(tab.layout)) {
        const focus =
          leaf.id === st.activePaneId && tab.id === st.activeTabId ? " [焦点]" : "";
        rows.push(
    ...
    return {
      ok: true,
      result: rows.length
        ? `${rows.join("\n")}\n\n引用规则：跨标签必须用完整 T{标签}:#{序号}（如 T1:#1），不要只用 #1（# 在每标签独立从 1 起，会打到当前焦点标签）。`
        : "（无窗格）",
    };
  }
  ...
  if (name === "close_pane") {
    const target = resolveTargetPane(st, args);
    if (!target.leaf) {
      return { ok: false, result: `窗格 ${target.label} 不存在` };
    }
    const leaf = target.leaf;
    // Find owning tab for "only one pane" check
    let owner = st.tabs.find((t) =>
      collectLeaves(t.layout).some((l) => l.id === leaf.id),
    );
    if (owner && collectLeaves(owner.layout).length <= 1) {
      return {
        ok: false,
        result: `标签「${owner.title}」仅剩一个窗格，无法关闭（可关闭整个标签）。`,
      };
    }
    st.closePane(leaf.id);
    return { ok: true, result: `已关闭窗格 ${paneLabelOf(st, leaf)}` };
  }
  ...
  if (name === "focus_pane") {
    ...
    if (!target.explicit) {
      return {
        ok: false,
        result: '缺少目标：请传 pane="T1:#1" 或 serial。跨标签必须用 pane。',
      };
    }
    if (!target.leaf) {
      return {
        ok: false,
        result: `窗格 ${target.label} 不存在。请先 list_panes 查看 T{标签}:#{序号}。`,
      };
    }
    activatePane(st, target.leaf);
    return {
      ok: true,
      result: `已聚焦 ${paneLabelOf(useWorkbenchStore.getState(), target.leaf)}（${target.leaf.shellKey} · ${target.leaf.cwd}）`,
    };
  }
  ...
  if (name === "read_pane") {
    ...
    if (!leaf) {
      return {
        ok: false,
        result: `窗格 ${target.label} 不存在。请先 list_panes，跨标签用 pane="T1:#1"。`,
      };
    }
    ...
    return {
      ok: true,
      result: `${paneLabelOf(st, leaf)} ${leaf.shellKey} ${leaf.cwd}\n${text}`,
    };
  }
  ...
  if (name === "run_command") {
    ...
    // Prefer pane="T1:#1"; bare serial is active-tab-first (legacy / same-tab only)
    const target = resolveTargetPane(st, args);
    let leaf = target.leaf;
    if (!leaf) {
      return {
        ok: false,
        result: `目标窗格不存在 (${target.label})。跨标签请用 pane="T1:#1"（先 list_panes）。`,
      };
    }

    // Wait for live PTY (create is async; leaf.ptyId is often empty)
    const leafIdFixed = leaf.id;
    let ptyId = getLivePtyId(leaf.id) || leaf.ptyId || null;
    for (let i = 0; i < 20 && !ptyId; i++) {
      await sleep(150);
      // Re-resolve by id (not serial) so we never jump to another tab's #1
      const stNow = useWorkbenchStore.getState();
      for (const tab of stNow.tabs) {
        const L = collectLeaves(tab.layout).find((x) => x.id === leafIdFixed);
        if (L) {
          leaf = L;
          break;
        }
      }
      ptyId = getLivePtyId(leafIdFixed) || leaf.ptyId || null;
    }
    if (!ptyId) {
      ...
      return {
        ...
        result:
          n === 0
            ...
            : `窗格 ${paneLabelOf(st, leaf)} 无 live PTY（活动会话 ${n} 个）。请点一下该窗格。未执行: ${command}`,
      };
    }
    ...
    if (!decision.run) {
      ...
      return {
        ok: true,
        result: `已按「仅插入」写入 ${paneLabelOf(st, leaf)}（未回车）: ${command}`,
      };
    }
        ...
        if (b && b.startedAt >= t0 - 1500) {
          ...
          if (!b.running) {
            ...
            return {
              ok: true,
              result: [
                `已在 ${paneLabelOf(st, leaf)} 执行并等到结束: ${command}`,
...
export async function runAgentToolLoop(opts: {
  ...
  for (let r = 0; r < maxRounds; r++) {
    ...
    for (const tc of res.toolCalls) {
      ...
      if (!exec.ok) {
        const n = (failStreak.get(streakKey) || 0) + 1;
        failStreak.set(streakKey, n);
        if (n >= 2) {
          const stopMsg =
            `同一工具调用连续失败 ${n} 次，已停止重复：${tc.name}(${argsPreview})。` +
            ` 请改用 list_panes + pane="T1:#1" 明确目标，或向用户说明失败原因，不要再重试相同参数。`;
          messages.push({
            role: "tool",
            tool_call_id: tc.id,
            content: `${exec.result}\n\n[系统] ${stopMsg}`,
      ...
      // Nudge wrong-pane: command landed on unexpected shell
      if (
        tc.name === "run_command" &&
        /不存在|跨标签|list_panes/i.test(exec.result) &&
        !/pane=/.test(tc.arguments || "")
      ) {
        toolContent +=
          '\n\n[系统提示] 窗格序号 #N 在每个标签独立从 1 起。跨标签请先 list_panes，再用 pane="T1:#1" 定位，不要只用 serial=1。';
      }
      messages.push({
        role: "tool",
        tool_call_id: tc.id,
