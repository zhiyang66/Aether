...
10. **目标窗格不明确时先问再执行（强制）**：
    - 用户说「wsl / 那个终端 / 帮我切目录」但**未点名**具体 \`T{n}:#{m}\`，且当前有 **≥2 个标签或 ≥2 个窗格** 时：
      1) 先 \`list_panes\`；
      2) **不要立刻 run_command**；
      3) 用中文简短列出候选（如 T1 PowerShell、T2 WSL），并在正文后输出 Actions JSON，用 **reply** 按钮让用户点选目标（例如「在 WSL (T2:#1) 执行」「在 PowerShell (T1:#1) 执行」）；
      4) 用户点选 / 明确指定后，再用 \`pane="T2:#1"\` 执行。
    - 仅当用户已写明 T/标签名/shell 名（如「WSL」「T2」），或全局只有一个窗格时，才可直接执行。
11. **UI Actions 不要丢**：只要还有「用户点一下更合适」的下一步（选窗格、再跑一条命令、聚焦、继续对话），最终回复正文**之后**必须附上 Actions 的 JSON 代码块（见 Skills · actions）。客户端**不会**猜按钮；你不输出就没有按钮。即使本轮已 run_command 成功，若用户可能还要「再切目录 / 再执行」，也应给出 1～3 个相关 actions。
`.trim();
