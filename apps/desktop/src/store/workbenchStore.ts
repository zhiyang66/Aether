...
type WorkbenchState = {
  ...
  /** Prefer active tab (serials restart at #1 per tab). */
  resolveSerial: (serial: number) => LeafPane | null;
  /**
   * Resolve pane by cross-tab ref: "T1:#1", "t2:1", "#1", or plain "1".
   * Tab index is 1-based left-to-right order in the tab bar.
   */
  resolvePaneRef: (ref: string) => LeafPane | null;
...
export const useWorkbenchStore = create<WorkbenchState>((set, get) => {
  return {
    ...
    resolvePaneRef: (ref) => {
      ...
      if (!raw) return null;
      const s = get();
      // T{tab}:#{serial}  or  t{tab}:{serial}  or  T{tab}:#N
      const m = raw.match(/^t(\d+)\s*[:：]\s*#?(\d+)$/i);
      if (m) {
        const tabIdx = Number(m[1]) - 1;
        const serial = Number(m[2]);
        const tab = s.tabs[tabIdx];
        if (!tab || !Number.isFinite(serial)) return null;
        return findLeafBySerial(tab.layout, serial);
      }
      // #N or plain N → active-tab-first serial lookup
      const m2 = raw.match(/^#?(\d+)$/);
      if (m2) return get().resolveSerial(Number(m2[1]));
      return null;
    },
